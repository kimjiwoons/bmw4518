...
# ============================================
# UI 요소 캐시 설정
# 고정 UI 요소의 위치를 캐시하여 속도 개선
# 삼성 브라우저 제외 (좌표 기반이라 XML 덤프 안 함)
# ============================================
ELEMENT_CACHE_CONFIG = {
    ...
    # 캐시할 요소 목록 (resource-id 기준)
    # 크롬, 파이어폭스, 오페라, 엣지에서 사용
    # 삼성은 좌표 기반이라 해당 없음
    "cacheable_elements": [
        "MM_SEARCH_FAKE",  # 네이버 메인 검색창 (가짜)
        "query",           # 검색 모드 입력창
        "nx_query",        # 검색 결과 페이지 검색창
    ],

    # ──────────────────────────────────────────
    # 기기별 파일 캐시 설정 (브라우저 첫 실행 버튼 등)
    # 기기마다 브라우저 버전이 달라 위치가 다를 수 있음
    # 파일을 삭제하면 다시 덤프하여 캐시함
    # ──────────────────────────────────────────
    # 캐시 파일 저장 디렉토리 (기본: adb/cache/)
    "cache_dir": "cache",

    # 캐시할 텍스트 요소 목록 (브라우저별 첫 실행 버튼)
    # 기기+브라우저+텍스트를 키로 파일에 저장
    "cacheable_text_elements": {
        "chrome": [
