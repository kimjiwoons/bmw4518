    ...
    *,
    ...
    try:
        ...
        if "Executable doesn't exist" in message:
            friendly = ("Playwright browser executable not found. Run `playwright install` to download "
                        "the required browser binaries.")
        else:
            friendly = f"Browser launch failed: {message}"
        return Result(target.url, "fail", friendly)
    ...
    *,
    ...
    async with async_playwright() as playwright:
        ...
        for target in iter_targets(config_path):
            ...
            if (
                result.status == "fail"
                and result.message.startswith("Playwright browser executable not found")
            ):
                logging.error("Playwright browser binaries are missing. Run `playwright install` and rerun the checker.")
                break
        return results
