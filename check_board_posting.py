...
an input configuration that can either be a JSON file with fine-grained
selectors or a simple text file containing one board URL per line.  For each
URL the script records a result entry that includes a status (``success`` when
the form opens, otherwise ``fail``) and the detected message that explains the
failure reason.  The aggregated results are written to a CSV file while a
plain-text failure report lists every URL that produced a popup with the
associated message.
...
def iter_targets(config_path: Path) -> Iterable[Target]:
    """Yield :class:`Target` objects described by ``config_path``.

    The file may be a JSON array of objects (for advanced selector overrides) or
    ...
    if not urls:
        raise ValueError("URL list is empty; provide at least one target")

    for url in urls:
        yield Target(url=url)
...
def write_failure_report(results: Sequence[Result], output_path: Path) -> None:
    """Write a text file containing one ``url -> message`` entry per failure."""
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Destination text file for failures in 'url -> message' format.",
    )
    parser.add_argument(
        "--timeout",
        type=float,
...
class CheckerGUI:
    ...
    def _save_results(self) -> None:
        ...
        try:
            ...
            failure_path = csv_path.with_suffix(".failures.txt")
            write_results(self.results, csv_path)
            write_failure_report(self.results, failure_path)
            messagebox.showinfo(
                "Checker",
                "Saved results to\n"
                f"- {csv_path}\n"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    write_failure_report(results, args.failure_log)
    logging.info(
        "Wrote %d result rows to %s (failures: %s)",
        len(results),
        args.output,
        args.failure_log,
    )
